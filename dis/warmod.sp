...
public OnPluginStart()
{
	...
	g_h_chat_prefix = CreateConVar("wm_chat_prefix", "WarMod", "Change the chat prefix. Default is WarMod", FCVAR_PROTECTED);
...
//	g_h_notify_version = CreateConVar("wm_version_notify", WM_VERSION, WM_DESCRIPTION, FCVAR_NOTIFY|FCVAR_REPLICATED);
	...
	g_h_competition = CreateConVar("wm_competition", "WarMod ZUBAT", "Name of host for a competition. eg. ESEA, Cybergamer, CEVO, ESL", FCVAR_PLUGIN);
...
stock LiveOn3Override()
{
	if (!g_half_swap)
	{
		...
		PrintToChatAll("\x01 \x09[\x04%s\x09]\x01 %t \x0EWarMod [ZUBAT]", CHAT_PREFIX, "Powered By");
		g_half_swap = true;
		return true;
	}
...
public Action:AdvertGameTech(Handle:timer, any:client)
{
	...
	{
		PrintToChat(client, "\x01 \x09[\x04%s\x09]\x01 %t \x03WarMod [ZUBAT]", CHAT_PREFIX, "Powered By");
	}
}
...
public Action:AdvertGameTechSpecs(Handle:timer)
{
	for (new i = 1; i <= MaxClients; i++)
	{
		...
		{
			PrintToChat(i, "\x01 \x09[\x04%s\x09]\x01 %t \x03WarMod [ZUBAT]", CHAT_PREFIX, "Powered By");
		}
	}
}
...
public Action:ShowPluginInfo(Handle:timer, any:client)
{
	if (client != 0 && IsClientConnected(client) && IsClientInGame(client))
	{
		...
		PrintToConsole(client, "");
		PrintToConsole(client, "Created by Twelve-60 and updated by Versatile [ZUBAT]");
		PrintToConsole(client, "");
